    ...
    return (
        <div className={`w-72 h-full p-6 text-gray-600 ${isOpen ? 'block bg-white md:bg-gray-100 z-50 md:z-0 h-screen md:h-auto fixed md:relative top-0 md:top-auto' : 'hidden'} md:block`}>
            <div className="flex flex-col space-y-8">
                <div className="text-2xl mb-10 flex flex-row space-x-3 items-center cursor-pointer" onClick={() => history.push('/')}>
                ...
                </div>
                {
                    menus.map((menu, idx) => {
                        return (
                            <div onClick={() => history.push({ pathname: menu.route })} key={idx} className="flex flex-row space-x-3 items-center cursor-pointer">
